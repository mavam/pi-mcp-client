...
for (const scenario of ["store-failure", "invalid-grant", "malformed-error", "success-error"] as const) {
  test(`SDK refresh regression: ${scenario}`, async () => {
    ...
    const server = Bun.serve({ hostname: "127.0.0.1", port: 0, fetch(request) {
      ...
      if (path === "/token") {
        ...
        if (scenario !== "store-failure") return Response.json({ error: "invalid_grant", error_description: "private-payload" }, {
          status: scenario === "success-error" ? 200 : 400,
        });
        return Response.json({ access_token: "private-new-token", refresh_token: "private-rotated", token_type: "Bearer" });
      }
    ...
    try {
      await expect(auth(provider, { serverUrl: `${base}/mcp`, fetchFn: privateOAuthFetch })).rejects.toThrow(
      ...
      if (scenario === "store-failure") expect(warnings).toEqual([]);
